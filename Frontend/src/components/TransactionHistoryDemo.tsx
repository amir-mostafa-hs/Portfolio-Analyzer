...
import { mockTransactions, TransactionMock } from "@/data/mockData";
import { cn } from "@/lib/utils";

const TransactionHistoryDemo = () => {
  const getTransactionIcon = (type: TransactionMock["type"]) => {
    switch (type) {
      case "buy":
  ...
  const getTransactionColor = (type: TransactionMock["type"]) => {
    switch (type) {
      case "buy":
        return "text-success";
      case "sell":
  ...
  const getStatusBadge = (status: TransactionMock["status"]) => {
    switch (status) {
      case "completed":
        return (
