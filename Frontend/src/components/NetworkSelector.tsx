...
const NetworkSelector = () => {
  const [selectedNetworks, setSelectedNetworks] = useState<string[]>(["bsc"]);
          ...
          {mockNetworks.map((network) => (
            <div
              ...
              className={cn(
                ...
                network.available ? "" : "cursor-not-allowed"
              )}
              onClick={() => {
            ...
            >
              <div className="flex items-center space-x-3">
                <div className="flex items-center justify-center">
                  <div
                    className={`w-10 h-10 rounded-full flex items-center justify-center bg-[${network.color}]`}
                  >
                    <img
                      src={network.icon}
                      alt={network.name}
        ...
        <div className="mt-6 pt-4 border-t border-primary/10">
          <div className="flex justify-between items-center">
            <span className="text-sm text-muted-foreground">
              {selectedNetworks.length} of {mockNetworks.length} networks
              selected
            </span>
            <Button
              variant="ghost"
